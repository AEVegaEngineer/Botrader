...
interface PriceChartProps {
  ...
  onToggleMACD?: (show: boolean) => void;
}
...
export function PriceChart({
  ...
  // Sync local indicators state with showSMA, showEMA, showBB, showRSI, and showMACD props
  useEffect(() => {
  ...
  useEffect(() => {
    setIndicators((prev) => ({ ...prev, macd: showMACD }));
  }, [showMACD]);
  ...
  const handleToggle = (indicator: string, value: boolean) => {
    ...
    if (indicator === "macd" && onToggleMACD) {
      onToggleMACD(value);
    }
  };
    ...
    <Grid>
      <Grid.Col span={{ base: 12, md: 9 }}>
        <Paper withBorder p="md" radius="md" h="100%">
          ...
          {/* MACD Sub-Chart */}
          {showMACD && macdData.length > 0 && isMounted && (
            <div style={{ width: "100%", marginTop: "16px" }}>
              <Text size="sm" fw={600} mb="xs" c="dimmed">
                MACD (12, 26, 9)
              </Text>
