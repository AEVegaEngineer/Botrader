...
async def get_rsi(
    ...
    """Get Relative Strength Index from candle_indicators table"""
    try:
        from app.core.database import engine
        from sqlalchemy import text
        ...
        query = text(f"""
            WITH recent_indicators AS (
                SELECT time, rsi_14 as rsi_value
                FROM candle_indicators
                WHERE symbol = :symbol
                  AND time >= :start_time
            ...
            SELECT 
                time_bucket('{pg_interval}'::interval, time) AS time,
                LAST(rsi_value, time) AS rsi_value
            FROM recent_indicators
            GROUP BY time_bucket('{pg_interval}'::interval, time)
            ORDER BY time DESC
            LIMIT :limit
        ...
        data = [
            {
                "timestamp": row.time.isoformat(),
                "value": float(row.rsi_value)
            }
            for row in rows if row.rsi_value is not None
        ]
